...
def inference(input_text: str, part: int, Passed: list[int]):
    ...
    若段落不符合規定，請回答：「檢查未通過！原因如下：」並列出支持的理由，請勿覆述段落內容。
...
condition = ["",
            "內容規定：請說明使用的作業系統、語言、套件(函式庫)、預訓練模型、額外資料集等。如使用預訓練模型及額外資料集，請列出來源。",
            "內容規定：說明演算法設計、模型架構與模型參數，包括可能使用的特殊處理方式。",
            "內容規定：說明做法不一樣之處，只要有一點點即可，模糊也算通過。這一段寬鬆檢查即可，不需要太明確也不須要具體",
            "內容規定：說明對資料的處理或擴增的方式，例如對資料可能的刪減、更正或增補。這一段寬鬆檢查即可，不需要太明確也不須要夠具體",
            "內容規定：說明模型的訓練方法與過程。",
